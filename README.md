...
### つぶやきの生成プロセス(無料のローカルLLM Ollamaを活用)

**設計方針**: 以前は「下書き全体をLLMに渡して丸ごと書き直させる」方式でしたが、
小型の無料モデルには制約が多すぎて品質が不安定になりがちでした
(表記ゆれや、【下書き】のような余計なラベルが混入する原因にもなっていました)。

そこで、**LLMの役割を「ニュースの本質を一文で言い当てること」だけに絞り**、
引用文・出典・URLは常にPython側が機械的に(=確実に)組み立てる方式に
変更しています。処理の流れは常に次の4段階です:

```
1. 箴言を選ぶ              … 常にPythonが決定的に選ぶ(改変されない)
2. 分析文を1文だけ得る      … LLM(失敗時はテンプレートに自動フォールバック)
3. 分析文+引用+出典を結合   … 常にPythonが機械的に結合(改変されない)
4. 先頭に元記事URLを付加    … 常にPythonが決定的に付加(改変されない)
```

LLMが失敗したり、おかしな出力をしても、その影響は「分析文1文」に
限定されます。引用の正確さや出典表記が崩れることはありません。

分析文の生成は以下の優先順で試み、失敗したら自動的に次へフォールバックします:
...
2. Anthropic API(ANTHROPIC_API_KEY設定時のみ)
   ↓ 失敗時
3. テーマ別テンプレート(必ず成功する最終フォールバック)
```

`.github/workflows/tweet.yml` には既にOllamaのセットアップ手順
(インストール→起動→軽量モデル `qwen2.5:3b` の取得)が含まれており、
**追加設定なしでそのまま動作**します。

**使用モデルを変更したい場合**は、`.github/workflows/tweet.yml` 内の
`ollama pull qwen2.5:3b` と `OLLAMA_MODEL: "qwen2.5:3b"` の2箇所を、
Ollamaが対応する他の軽量モデル名(例: `gemma2:2b`, `llama3.2:1b` など)に
書き換えてください。モデルが大きいほど分析文の質は上がりますが、
GitHub Actions上でのダウンロード・生成時間は長くなります。

**ローカルPC上でOllamaを使う場合**(`bot.py` を手元で実行する場合など)は、
[Ollama公式サイト](https://ollama.com)からインストールし、
`ollama serve` を起動した状態で `ollama pull qwen2.5:3b` を実行しておけば、
同様に自動でOllamaモードが使われます。

### 箴言(マキャベリの引用)のローテーション

同じ箴言ばかりが繰り返し引用されることを防ぐため、以下のルールで
自動的にローテーションさせています(`geopolitical_scorer.py` によるニュースの
テーマ分類 → そのテーマに合う箴言群の中から、下記の優先順位で1つを選定):

1. **最も長く使われていない箴言を最優先**(直近5回の投稿の中で使っていない、
   または最も古くに使ったもの)。箴言が2〜3件しかないテーマでも、
   同じものが連続で選ばれることはありません
2. 同率の場合は、**通算の使用回数が最も少ないもの**を優先
3. さらに同率ならランダムに選択

使用履歴は `data/quotes.json` 内の各箴言の `id` を使って
`logs/recent_quotes.json`(直近5件)・`logs/quote_usage.json`(通算回数)に
記録され、GitHub Actionsの実行のたびにコミットされて引き継がれます。

**箴言をさらに増やしたい場合**は、`data/quotes.json` の該当テーマの配列に、
他の箴言と重複しない `id`(例: 既存が `AD1`, `AD2` なら次は `AD3`)を付けて
追記するだけです。箴言が増えるほど、ローテーションの幅が広がり、
同じ引用に当たる頻度がさらに下がります。

#### テーマ分類の精度について(2026年改訂)

以前は「ニュースに含まれるキーワードの一致数が最も多いテーマ」を採用しており、
同点の場合は特定のテーマ(military_alliance)が構造上常に勝つ実装になっていました。
その結果、「米軍とイランの攻撃応酬」のような**実際の武力衝突を報じる記事**まで
「同盟・依存」テーマに誤分類され、そのテーマの箴言(2件)ばかりが
繰り返し引用される事態が発生していました。

現在は `geopolitical_scorer.py` の `TOPIC_PRIORITY` により、
「実際の武力攻撃・侵攻・封鎖」を最優先で判定し、それが無い場合にのみ
「同盟・駐留」等のより一般的なテーマを採る**優先順位方式**に改めています。
これにより、記事の実態に即したテーマ分類がなされ、結果として
箴言の使用もテーマ単位でまんべんなく分散するようになりました。
